              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                ...
                {(data.mascota.temperamento || data.mascota.alergias || data.mascota.restricciones || data.mascota.vacunas) && (
                  <div className="mt-3 pt-3 border-t border-gray-200 space-y-1 text-sm">
                    {data.mascota.temperamento && (
                      <p><span className="text-gray-500">Temperamento:</span> {data.mascota.temperamento}</p>
                    )}
                    {data.mascota.alergias && (
                      <p><span className="text-gray-500">Alergias:</span> {data.mascota.alergias}</p>
                    )}
                    {data.mascota.restricciones && (
                      <p><span className="text-gray-500">Restricciones:</span> {data.mascota.restricciones}</p>
                    )}
                    {data.mascota.vacunas && (
                      <p><span className="text-gray-500">Vacunas:</span> {data.mascota.vacunas}</p>
                    )}
                  </div>
                )}
              </div>
                            ...
                            <div className="flex gap-2 overflow-x-auto pb-2">
                              {ficha.fotos.map((foto) => (
                                <button
                                  ...
                                  className="relative w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 hover:opacity-80 transition-opacity border-2 border-gray-200"
                                >
                                  <img
                                    src={foto.url}

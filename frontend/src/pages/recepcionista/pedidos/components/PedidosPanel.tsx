...
import { recepcionistaPedidosService } from '../services/recepcionista.pedidos.service';
...
export const PedidosPanel = () => {
  ...
  const loadPedidos = useCallback(async () => {
    try {
      setIsLoading(true);
      const data = await recepcionistaPedidosService.listar({ estado, page });
  ...
  const handleConfirmar = async (pedido: Pedido) => {
    try {
      ...
      await recepcionistaPedidosService.confirmar(pedido.idPedido);
  ...
  const handlePagar = async () => {
    ...
    try {
      ...
      const result = await recepcionistaPedidosService.pagar(pedidoPago.idPedido, medioPago);

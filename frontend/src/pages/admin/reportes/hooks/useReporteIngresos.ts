// src/pages/admin/reportes/hooks/useReporteIngresos.ts
import { useState, useCallback } from 'react';
import { useToast } from '../../../../hooks/useToast';
import { adminReportesService } from '../services/admin.reportes.service';
import type { ReporteIngresosResponse } from '../../../../services/types/admin';
...
export const useReporteIngresos = () => {
  const [data, setData] = useState<ReporteIngresosResponse | null>(null);
  ...
  const generarReporte = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await adminReportesService.getReporteIngresos({
      ...
      showToast(getErrorMessage(error, 'Error al generar reporte de ingresos'), 'error');
    } finally {
      setIsLoading(false);
    }

...
import { Paso4Slot } from './Paso4Slot';
import { Paso5Confirmacion } from './Paso5Confirmacion';
import { useAgendaRecepcion } from '../hooks/useAgendaRecepcion';
import { NuevoClienteModal } from './NuevoClienteModal';
import { NuevaMascotaModal } from './NuevaMascotaModal';
import { toDateInputValue } from '../utils/date';

interface ModalNuevaCitaWizardProps {
  isOpen: boolean;
  onClose: () => void;
  fechaInicial?: string;
...
export const ModalNuevaCitaWizard = ({
  ...
  const handleConfirmar = async () => {
    ...
    if (result) { onCitaCreada(); onClose(); }
  };
  ...
  return (
    <>
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 10, padding: 8, display: 'flex', color: 'rgba(255,255,255,0.8)',
                }}
              >
          ...
          >
            <button
              ...
              style={{
                ...
                backgroundColor: '#fff', color: '#374151', fontSize: 13, fontWeight: 600,
              ...
              {STEPS.map(s => (
                <div
                  ...
                  style={{
                    ...
                    backgroundColor:
                      s.number < currentStep ? '#22c55e' :
                      s.number === currentStep ? '#2563eb' : '#e2e8f0',
      ...
      {/* Sub-modales — renderizan en <body> mediante createPortal (z-index 9999) */}
      <NuevoClienteModal
        isOpen={showNuevoClienteModal}
        onClose={() => setShowNuevoClienteModal(false)}
        onClienteCreado={cliente => { setSelectedCliente(cliente); setShowNuevoClienteModal(false); }}
      />
      <NuevaMascotaModal
        isOpen={showNuevaMascotaModal}
        onClose={() => setShowNuevaMascotaModal(false)}
        clienteId={selectedCliente?.id || 0}
        onMascotaCreada={mascota => { setSelectedMascota(mascota); setShowNuevaMascotaModal(false); }}
      />
    </>
  );
};